...
// ─── BABYCARE-ONLY METADATA (isolated, never shared with OralCare/HairCare) ────
const BABYCARE_BRAND        = "NIGOODA";
const BABYCARE_TITLE        = "Baby Care Intelligence";
const BABYCARE_BADGE        = "Pediatric Analysis";
const BABYCARE_HERO_LABEL   = "👶 BABY CARE ANALYSIS";
const BABYCARE_GUIDANCE_MSG = "Check compatibility scores for your baby's skin type";
...
function detectProductType(md: string): string {
  const lower = md.toLowerCase();
  if (lower.includes("sunscreen") || lower.includes("uv")) return "sunscreen";
  if (lower.includes("powder")) return "powder";
  if (lower.includes("shampoo")) return "shampoo";
  if (lower.includes("wash") || lower.includes("soap")) return "wash";
...
const ING_GROUPS: Record<string, string[]> = {
  Emollients       : ["glycerin", "ceramide", "squalane", "jojoba", "almond", "coconut", "sunflower", "mineral oil", "petrolatum", "lanolin"],
  Cleansers        : ["sodium lauryl", "sls", "surfactant", "cocamidopropyl", "decyl", "coco-glucoside"],
  UVFilters        : ["titanium dioxide", "zinc oxide", "octinoxate", "avobenzone", "oxybenzone", "homosalate", "uva", "uvb"],
  Preservatives    : ["phenoxyethanol", "paraben", "methylisothiazolinone", "benzoate", "potassium sorbate", "sodium benzoate"],
  Soothing         : ["aloe", "chamomile", "oat", "panthenol", "allantoin", "hyaluronic", "calendula", "bisabolol"],
  Moisturizers     : ["glycerin", "sorbitol", "urea", "lactic acid", "xylitol"],
  Actives          : ["salicylic", "benzoyl peroxide", "sulfur", "hydroquinone", "retinol", "niacinamide"],
  Fragrance        : ["parfum", "fragrance", "essential oil", "extract", "scent"],
  Antimicrobials   : ["zinc pyrithione", "salicylic", "tea tree", "eucalyptus", "thymol", "triclosan"],
  Colorants        : ["dye", "colorant", "ci ", "mica", "iron oxide", "carmine"],
};

const ING_COLORS: Record<string, ColorSet> = {
  Emollients       : { bg:"#F0FDF4", dot:"#22C55E", text:"#14532D", border:"#DCFCE7" },
  Cleansers        : { bg:"#FFF7ED", dot:"#F97316", text:"#7C2D12", border:"#FED7AA" },
  UVFilters        : { bg:"#FEF3C7", dot:"#FBBF24", text:"#92400E", border:"#FDE68A" },
  Preservatives    : { bg:"#F8FAFC", dot:"#94A3B8", text:"#334155", border:"#E2E8F0" },
  Soothing         : { bg:"#ECFDF5", dot:"#10B981", text:"#064E3B", border:"#A7F3D0" },
  Moisturizers     : { bg:"#F0F9FF", dot:"#0EA5E9", text:"#0C4A6E", border:"#BAE6FD" },
  Actives          : { bg:"#F5F3FF", dot:"#A855F7", text:"#4C1D95", border:"#DDD6FE" },
  Fragrance        : { bg:"#FFF1F2", dot:"#F43F5E", text:"#881337", border:"#FECDD3" },
  Antimicrobials   : { bg:"#FEE2E2", dot:"#DC2626", text:"#7F1D1D", border:"#FECACA" },
  Colorants        : { bg:"#F5F3FF", dot:"#8B5CF6", text:"#3730A3", border:"#E9D5FF" },
...
function inferRisk(name: string): number {
  const lower = name.toLowerCase();
  if (lower.includes("fragrance") || lower.includes("essential oil") || lower.includes("parfum")) return 5;
  if (lower.includes("sodium lauryl") || lower.includes("sls")) return 4;
  if (lower.includes("paraben") || lower.includes("methylisothiazolinone")) return 4;
  if (lower.includes("phenoxyethanol")) return 3;
  if (lower.includes("preservative") || lower.includes("benzoate")) return 3;
  if (lower.includes("colorant") || lower.includes("ci ")) return 3;
  if (lower.includes("emollient") || lower.includes("moisturizer")) return 1;
  if (lower.includes("soothing") || lower.includes("aloe")) return 1;
  return 2;
}
...
// ─── MAIN COMPONENT ───────────────────────────────────────────────────────────
interface BabyCareProps {
  markdown: string;
}

export default function BabyCare({ markdown }: BabyCareProps) {
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ fontSize: 13, color: T.textFaint }}>Please provide a valid baby care analysis result.</div>
        </div>
      </div>
    );
  }
  ...
  const productType = detectProductType(markdown);
  ...
  // ── Pediatric compatibility sections ──
  const { lines: ageCompatLines,
          key: ageKey }              = findSection(sections, ["INFANT", "AGE", "NEWBORN"]);
  const { lines: zoneCompatLines,
          key: zoneKey }             = findSection(sections, ["ZONE", "AREA", "APPLICATION"]);
  const { lines: skinCondLines,
          key: skinKey }             = findSection(sections, ["SKIN CONDITION", "ATOPIC"]);

  const { lines: ltLines }           = findSection(sections, ["LONG-TERM USABILITY", "FREQUENCY"]);
  ...
  const naturalTruthMatch = Array.from(sections.entries()).find(([key]) =>
    key.toUpperCase().includes("NATURAL") && key.toUpperCase().includes("CLAIM")
  );
  ...
  const ageCompat     = parseCompatibility(ageCompatLines);
  const zoneCompat    = parseCompatibility(zoneCompatLines);
  const skinCompat    = parseCompatibility(skinCondLines);
  ...
  const TABS: Tab[] = [
    ...
    { id: "compatibility",   label: "Compatibility",    icon: "👶", enabled: ageCompat.length > 0 || zoneCompat.length > 0 || skinCompat.length > 0 },
  ...
  return (
    <div style={{
      ...
      {/* ══ HEADER BAR (BabyCare-only metadata) ══ */}
      <div style={{
        ...
        <span style={{ fontWeight: 800, fontSize: 12.5, letterSpacing: "0.08em", color: T.indigo }}>{BABYCARE_BRAND}</span>
        <span style={{ fontSize: 12, color: T.borderMid, margin: "0 4px" }}>·</span>
        <span style={{ fontSize: 12, color: T.textFaint, fontWeight: 500 }}>{BABYCARE_TITLE}</span>
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{
            fontSize: 10.5, color: T.textFaint, background: T.surfaceAlt,
            padding: "3px 10px", borderRadius: 99, border: `1px solid ${T.border}`, fontWeight: 600,
          }}>
      ...
      {/* ══ SMART GUIDANCE BANNER ══ */}
      <SmartGuidanceBanner message={BABYCARE_GUIDANCE_MSG} />
      ...
      <div style={{ maxWidth: 980, margin: "0 auto", padding: "22px 22px" }}>
        ...
        {currentTab === "scores" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {specScores.length > 0 && (
              <SectionCard title="Specialized Pediatric Performance" icon="🧪">
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(230px, 1fr))", gap: 10 }}>
        ...
        {currentTab === "insights" && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 14 }}>
            {strengths.length > 0 && (
              <SectionCard title="Infant Safety Strengths" icon="✓" accent={T.green}>
            ...
            {weaknesses.length > 0 && (
              <SectionCard title="Safety Concerns" icon="−" accent={T.red}>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {weaknesses.map((s, i) => (
                    <div key={i} style={{
                      display: "flex", gap: 10, alignItems: "flex-start",
        ...
        {/* ════ COMPATIBILITY (Pediatric Age/Zone/Condition) ════ */}
        {currentTab === "compatibility" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>

            {/* Age Compatibility */}
            {ageCompat.length > 0 && (
              <SectionCard title="Age Compatibility" icon="👼">
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: 9 }}>
                  {ageCompat.map((item, i) => <CompatCard key={i} item={item} />)}
                </div>
              </SectionCard>
            )}
            ...
            {/* Zone Compatibility */}
            {zoneCompat.length > 0 && (
              <SectionCard title="Application Zone Safety" icon="🗺">
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: 9 }}>
                  {zoneCompat.map((item, i) => <CompatCard key={i} item={item} />)}
                </div>
              </SectionCard>
            )}
            ...
            {skinCompat.length > 0 && (
              <SectionCard title="Skin Condition Suitability" icon="🧴">
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: 9 }}>
                  {skinCompat.map((item, i) => <CompatCard key={i} item={item} />)}
                </div>
              </SectionCard>
            )}
            ...
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <span style={{ fontSize: 11.5, color: T.textMid }}>Safe (≥3.0)</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {truthItems.length === 0 && herbalItems.length === 0 && (
              <div style={{
                ...
                <div style={{ fontSize: 13, color: T.textFaint }}>No natural claims or herbal evidence in this formulation.</div>
              </div>
            )}
          </div>
        )}

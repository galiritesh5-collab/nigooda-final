...
const tabSections = sections
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      {/* ---------------------------------------------------------- */}
      <div className="sticky top-0 z-10 -mx-1 px-1 bg-white/95 backdrop-blur-md border-b border-slate-200/80">
        <div
          ref={tabListRef}
          role="tablist"
          aria-label="Analysis report sections"
          onKeyDown={handleKeyDown}
          className="flex gap-1.5 overflow-x-auto py-3.5 -mb-px"
        >
          {tabSections.map((section, i) => {
            ...
            return (
              <button
                ...
                className={[
                  "whitespace-nowrap shrink-0 rounded-full px-4 py-2 text-xs font-medium transition-all duration-200 max-w-[180px] truncate active-press",
                  "focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:ring-indigo-600/20",
                  isActive
                    ? "bg-indigo-600 text-white shadow-sm font-semibold"
                    : "bg-slate-100/80 text-slate-600 hover:bg-slate-200/80 hover:text-slate-900 border border-transparent hover:border-slate-300/40",
                ].join(" ")}
              >
